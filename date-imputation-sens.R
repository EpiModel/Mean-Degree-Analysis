  ...
  for (j in 1:nrow(unique(ARTnet.wide[filter_var]))) {
    ...
    for (i in start_month_offset:end_month_offset) {
      ...
      assign(ongoing4, ifelse(is.na(ifelse(ARTnet.long.adjusted2$start.date.2 <=
                                           ...
                                           1, 
                                           ifelse(month(ARTnet.long.adjusted2$start.date.2) == 
                                                    month(get(ongoing.eval)) &
                                                    year(ARTnet.long.adjusted2$start.date.2) == 
                                                    year(get(ongoing.eval)), 
                                                  1, 0))), 0, 
                              ifelse(ARTnet.long.adjusted2$start.date.2 <=
                                            get(ongoing.eval) &
                                            ARTnet.long.adjusted2$end.date.2 >= get(ongoing.eval),
                                            1, 
                                     ifelse(month(ARTnet.long.adjusted2$start.date.2) == 
...
write.csv(all_max, "all_max.csv")


race.cat_max <- n_month_offset_max(0,12,'race.cat')
age.cat_max <- n_month_offset_max(0,12,'age.cat')
region_max <- n_month_offset(0,12,'REGCODE')
income_max <- n_month_offset_max(0,12,'HHINCOME')
income_max <- income_max %>% filter(!is.na(var_val) &
                              var_val < 77)
education_max <- n_month_offset_max(0,12,'HLEDUCAT_2')
education_max <- education_max %>% filter(!is.na(var_val))

partners_bi2_max <- n_month_offset_max(0,12,'partners_bi2')
partners_bi2_max <- partners_bi2_max %>% filter(!is.na(var_val))
...
n_month_offset_min <- function(start_month, end_month, filter_var='all',
                               output_type='df') {
  ...
  for (j in 1:nrow(unique(ARTnet.wide[filter_var]))) {
    ...
    for (i in start_month_offset:end_month_offset) {
      ...
      assign(ongoing4, ifelse(is.na(ifelse(month(ARTnet.long.adjusted2$start.date.2) == 
                                             month(get(ongoing.eval)) &
                                             year(ARTnet.long.adjusted2$start.date.2) == 
                                             year(get(ongoing.eval)), 
                                           0,
...
write.csv(all_min, "all_min.csv")

race.cat_min <- n_month_offset_min(0,12,'race.cat')
age.cat_min <- n_month_offset_min(0,12,'age.cat')
region_min <- n_month_offset_min(0,12,'REGCODE')
income_min <- n_month_offset_min(0,12,'HHINCOME')
income_min <- income_min %>% filter(!is.na(var_val) &
                                      var_val < 77)
education_min <- n_month_offset_min(0,12,'HLEDUCAT_2')
education_min <- education_min %>% filter(!is.na(var_val))

partners_bi2_min <- n_month_offset_min(0,12,'partners_bi2')
partners_bi2_min <- partners_bi2_min %>% filter(!is.na(var_val))
