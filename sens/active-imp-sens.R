# impute DK/refuse based on ptype

set.seed(100) # reproducibility
ARTnet.long <- ARTnet.long %>%  # ptype 3 must be inactive
  mutate(active = ifelse(is.na(ONGOING), 2, ONGOING),
  ) %>% # impute DK responses for active based on ptype
  rowwise() %>%
  mutate(active.imp = case_when(
    active==2 & ptype==2 ~ as.numeric(rbinom(1,1,0.5)),
    active==2 & ptype==3 ~ as.numeric(rbinom(1,1,0.1)),
    TRUE ~ active)
  ) %>%
  ungroup() %>% # reclassify ptype3 based on imputation
  mutate(ptype.imp = case_when(
    ptype==3 & active.imp==1 ~ 2,
    TRUE ~ ptype)
  )

table(ARTnet.long$ptype, ARTnet.long$ptype.alt)

table(ARTnet.long$ptype, ARTnet.long$ptype.imp)

table(ARTnet.long$ptype, ARTnet.long$ptype.imp.alt)


n_month_offset_imp <- function(start_month, end_month, filter_var='all',
                           output_type='df') {
...
all.imp <- n_month_offset_imp(0,12)
